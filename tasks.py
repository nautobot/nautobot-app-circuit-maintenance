...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_CIRCUIT_MAINTENANCE_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_circuit_maintenance.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_circuit_maintenance.compose_http_timeout,
    ...
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_circuit_maintenance.project_name}",
        f'--project-directory "{context.nautobot_circuit_maintenance.compose_dir}"',
    ]
    ...
    if (
        context.nautobot_circuit_maintenance.ephemeral_ports
        and context.nautobot_circuit_maintenance.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_circuit_maintenance.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_circuit_maintenance.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.nautobot_circuit_maintenance.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_circuit_maintenance.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_circuit_maintenance.compose_dir, "creds.env"),
        )
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_circuit_maintenance.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_circuit_maintenance"
...
@task(
    help={
        ...
        "root": "Run command as root (default: False)",
    },
)
def exec(context, service="nautobot", command="bash", file="", root=False):
...
@task(
    help={
        "version": "Version of Circuit Maintenance to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_circuit_maintenance'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_circuit_maintenance")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_circuit_maintenance.migrations"
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_circuit_maintenance/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Circuit Maintenance."""
    command = "nautobot-server generate_nautobot_circuit_maintenance_test_data"
    if database:
...
def tests(context, failfast=False, keepdb=False, no_input=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_circuit_maintenance.local):
