...
def _is_compose_included(context, name):
    return (
        ...
        in context.nautobot_circuit_maintenance.compose_files
    )
...
def docker_compose(context, command, **kwargs):
    ...
    for compose_file in context.nautobot_circuit_maintenance.compose_files:
        compose_file_path = os.path.join(
            context.nautobot_circuit_maintenance.compose_dir, compose_file
        )
...
def build(context, force_rm=False, cache=True):
    ...
    print(
        f"Building Nautobot with Python {context.nautobot_circuit_maintenance.python_ver}..."
    )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    dockerfile_path = os.path.join(
        context.nautobot_circuit_maintenance.compose_dir, "Dockerfile"
    )
    base_image = (
...
def pylint(context):
    ...
    command = (
        'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_circuit_maintenance'
    )
    run_command(context, command)
