...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_circuit_maintenance"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_circuit_maintenance") / Path("migrations")
